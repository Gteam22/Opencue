...
import '../context_builder/context_chips.dart';
...
class RecommendationsScreen extends StatefulWidget {
  ...
  /// The draft behind [situation], when the context came from the radial
  /// composer or a scan. Supplying it turns on the chip row and the *Adjust*
  /// button; without it the screen behaves exactly as before.
  final ContextDraft? draft;

  /// The scan's own values, so *Use scan* can restore them after edits.
...
class _RecommendationsScreenState extends State<RecommendationsScreen> {
  ...
  /// The context currently being shown. Starts as whatever was passed in and
  /// is replaced in place when the user adjusts it, so adjusting does not
  /// push another copy of this screen onto the stack.
  ContextDraft? _draft;
  ContextSnapshot? _situationOverride;
  RecommendationPreferences? _preferencesOverride;
  ...
  RecommendationResult _score() {
    ...
    return state.engine.recommend(
      context: _situationOverride ?? widget.situation,
      library: state.lines,
      preferences: _preferencesOverride ?? widget.preferences,
      recentlyShownIds: state.recentlyShownIds.union(_dismissed),
    );
  }

  /// Applies an adjusted draft and rescores without leaving the screen.
  ///
  /// A caution flag added here changes the result immediately, including
  /// flipping it to the advisory, which is the point of putting the control
  /// on this screen rather than a page away.
  void _applyDraft(ContextDraft draft) {
    setState(() {
      _draft = draft;
      _situationOverride = draft.toSnapshot();
      _preferencesOverride = RecommendationPreferences(
        desiredDirectness: draft.directness,
        preferredTones: draft.preferredTones,
      );
      // A new context is a new round, so previously swapped-out lines are
      // eligible again.
      _dismissed.clear();
      _result = _score();
    });
  ...
  Future<void> _adjust({String? branchId}) async {
    final draft = _draft;
          ...
          : ListView(
              children: <Widget>[
                if (_draft != null)
                  ContextChipRow(
                    draft: _draft!,
                    // Tapping a chip opens the menu at that branch, so
                    // changing one dimension never restarts from the root.
                    onChipTap: (branchId) => _adjust(branchId: branchId),
                  ),
                ContentColumn(
                  child: Column(
