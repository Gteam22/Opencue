      ...
      children: <Widget>[
        ...
        Wrap(
          ...
          children: <Widget>[
            ...
            // location and groupSize are non-nullable on ContextSnapshot and
            // default to "other" / "unknown" rather than being absent, so the
            // tag is only worth showing once it differs from that default.
            if (snapshot != null && snapshot.location != LocationTag.other)
              MetaTag(strings.location(snapshot.location)),
            if (snapshot != null && snapshot.groupSize != GroupSize.unknown)
