...
def parse_dart_enums():
    ...
    for match in re.finditer(r'enum\s+(\w+)\s*\{(.*?)\}', src, re.S):
        ...
        # Comments are stripped *before* the split on ';'. A doc comment
        # containing a semicolon would otherwise truncate the body, and
        # one containing a comma would hide the value after it.
        body = strip_dart_comments(body).split(';')[0]
