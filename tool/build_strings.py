...
add('about.privacyNoMic',
    'The microphone is used only while Listen Mode is on. OpenCue does not '
    'save raw audio.',
    'マイクはリッスンモードがオンの間だけ使用します。OpenCueは音声データを'
    '保存しません。')
...
# ---------------------------------------------------------------------------
# Conversation Assist and library extensions
#
# These strings were introduced with the multilingual conversation library.
# They live in the generator (rather than only in its Dart output) so CI's
# regenerate-and-diff check keeps the tables reproducible.
# ---------------------------------------------------------------------------
add('assist.accept', 'Mark as used', '使った一言として記録')
add('assist.adultConfirmBody', 'This can include sexual or explicit curated lines. You control the maximum boldness and can turn it off at any time.', '性的または露骨な一言が含まれる場合があります。大胆さの上限はいつでも変更できます。')
add('assist.adultConfirmTitle', 'Enable adult suggestions?', '大人向けの提案を有効にしますか？')
add('assist.adultContent', 'Adult suggestions', '大人向けの提案')
add('assist.adultContentHint', 'Off by default. Explicit lines never appear unless you opt in.', '初期設定はオフです。明示的に有効にしない限り、露骨な一言は表示されません。')
add('assist.adultEnable', 'Enable', '有効にする')
add('assist.autoSpeak', 'Auto Speak', '自動読み上げ')
add('assist.autoSpeakHint', 'Read the primary reply, then automatically resume listening.', '最初の返答を読み上げた後、自動的に聞き取りを再開します。')
add('assist.autoSpeakUnavailable', 'Speech playback is unavailable on this device.', 'この端末では音声読み上げを利用できません。')
add('assist.boldness', 'Maximum boldness', '最大の大胆さ')
add('assist.copy', 'Copy Japanese line', '日本語の一言をコピー')
add('assist.cuesFor', 'Cues for: “{0}”', 'この発話への候補：「{0}」')
add('assist.detected.english', 'EN', '英')
add('assist.detected.japanese', 'JA', '日')
add('assist.detected.korean', 'KO', '韓')
add('assist.detected.unknown', '—', '—')
add('assist.dismiss', 'Not useful', '役に立たなかった')
add('assist.error', 'Recognition stopped unexpectedly. Please try again.', '音声認識が停止しました。もう一度お試しください。')
add('assist.hearingSpeech', 'Voice detected — listening to this turn…', '声を検出しました。この発話を聞き取っています…')
add('assist.homeSubtitle', 'Hear the conversation and suggest what to say next', '会話を聞いて、次に言う一言を提案')
add('assist.initializing', 'Preparing speech recognition…', '音声認識を準備しています…')
add('assist.startingListener', 'Starting listener…', '音声認識を開始しています…')
add('assist.inputLanguage', 'Recognition', '認識言語')
add('assist.language.automatic', 'Auto / device language', '自動／端末の言語')
add('assist.language.both', 'Japanese + Korean (safe fallback)', '日本語＋韓国語（安全なフォールバック）')
add('assist.language.english', 'English', '英語')
add('assist.language.japanese', 'Japanese', '日本語')
add('assist.language.korean', 'Korean', '韓国語')
add('assist.listen', 'Listen', '聞く')
add('assist.listening', 'Listening now — tap Stop any time.', '聞き取り中です。いつでも停止できます。')
add('assist.lowConfidence', 'I may have misheard that. Check the transcript or listen again.', '聞き間違えた可能性があります。認識結果を確認するか、もう一度聞いてください。')
add('assist.more', 'More', '他の候補')
add('assist.noSpeech', 'I did not catch speech. Try again or type it below.', '音声を聞き取れませんでした。再試行するか、下に入力してください。')
add('assist.permissionDenied', 'Microphone permission was denied. Enable it in system settings, or type below.', 'マイクの許可がありません。端末設定で許可するか、文字で入力してください。')
add('assist.pipelineDiagnostics', 'Utterance pipeline diagnostics', '発話パイプライン診断')
add('assist.privacy', 'Privacy: the microphone is active only while Listen Mode is on. Raw audio is not stored by OpenCue; transcripts and this session history stay in memory and are cleared when you leave this screen.', 'プライバシー：マイクはリッスンモードがオンの間だけ動作します。OpenCueは音声データを保存しません。認識結果と履歴はメモリ内だけに保持され、この画面を閉じると消去されます。')
add('assist.ready', 'Tap when the other person starts speaking.', '相手が話し始める時にタップしてください。')
add('assist.readyAgain', 'Ready for another turn.', '次の発話を聞く準備ができました。')
add('assist.reel.flirty', 'FLIRTY', 'フラーティー')
add('assist.reel.funny', 'FUNNY', 'ユーモア')
add('assist.reel.standard', 'STANDARD', 'スタンダード')
add('assist.safeFallback', 'The meaning was uncertain, so these are low-pressure fallback options.', '意味の確信度が低いため、無理のない候補を表示しています。')
add('assist.sessionHistory', 'This session', 'このセッション')
add('assist.speaking', 'Speaking the primary response…', '最初の返答を読み上げています…')
add('assist.startListening', 'Start Listening', '聞き取りを開始')
add('assist.stop', 'Stop listening', '聞き取りを停止')
add('assist.stopListening', 'Stop Listening', '聞き取りを停止')
add('assist.stopping', 'Stopping…', '停止しています…')
add('assist.style', 'Response style', '返答スタイル')
add('assist.style.natural', 'Natural', '自然')
add('assist.subtitle', 'Keep listening across natural turns, understand what was said, and choose a natural reply.', '自然な会話の流れを継続して聞き取り、内容に合った返答を提案します。')
add('assist.suggestions', 'Suggested replies', '返答の候補')
add('assist.title', 'Conversation Assist', '会話アシスト')
add('assist.tone.bold', 'Bold', '大胆')
add('assist.tone.flirty', 'Flirty', 'フラーティー')
add('assist.tone.funny', 'Funny', '面白く')
add('assist.tone.gentleman', 'Gentleman', '紳士的')
add('assist.tone.natural', 'Natural', '自然')
add('assist.transcript', 'What I heard', '聞き取った内容')
add('assist.transcriptHint', 'You can also type or correct the conversation here.', '会話を直接入力したり、認識結果を修正したりできます。')
add('assist.unavailable', 'Speech recognition is unavailable on this device. You can still type the transcript.', 'この端末では音声認識を利用できません。文字入力は利用できます。')
add('assist.understanding', 'Understanding the conversation…', '会話の内容を理解しています…')
add('assist.understoodAs', 'Understood: {0}', '理解した内容：{0}')
add('assist.useEdited', 'Use edited transcript', '修正した内容を使う')
add('assist.waitingForSpeech', 'Waiting for speech…', '発話を待っています…')
add('boldness.explicit', 'Explicit', '明確な大人向け')
add('boldness.flirty', 'Flirty', 'フラーティー')
add('boldness.light', 'Light', 'ライト')
add('boldness.naughty', 'Naughty', 'ちょっとエッチ')
add('category.comebacks', 'Comebacks', '切り返し')
add('category.flirty', 'Flirty', 'フラーティー')
add('category.games', 'Games', 'ゲーム')
add('category.gentleman', 'Gentleman', '紳士的')
add('category.intimate', 'Intimate', '親密')
add('category.kissing', 'Kissing', 'キス')
add('category.naughty', 'Naughty', 'ちょっとエッチ')
add('category.playful', 'Playful', '遊び心')
add('category.questions', 'Questions', '質問')
add('category.witty', 'Witty', 'ウィット')
add('common.cancel', 'Cancel', 'キャンセル')
add('intentTester.detected', 'Detected intents', '検出された意図')
add('intentTester.input', 'Transcript to test', 'テストする認識結果')
add('intentTester.none', 'No intent passed its confidence threshold.', '確信度の基準を満たす意図はありませんでした。')
add('intentTester.run', 'Detect intent', '意図を判定')
add('intentTester.subtitle', 'Paste a transcript to inspect intent confidence and ranked replies.', '認識結果を貼り付けて、意図の確信度と返答候補を確認します。')
add('intentTester.title', 'Intent tester', '意図テスター')
add('library.boldness', 'Boldness', '大胆さ')
add('library.topics', 'Topics', 'トピック')
add('library.usageType', 'Line type', 'タイプ')
add('radial.conversationLibrary', 'Flirting library', 'フリーティング')
add('settings.japaneseTts', 'Read Japanese aloud', '日本語を音声で読み上げ')
add('settings.japaneseTtsNote', 'Show a speaker button to hear each Japanese line spoken.', '各日本語の行を音声で聞くための再生ボタンを表示します。')
add('tone.classy', 'Classy', '上品')
add('tone.confident', 'Confident', '自信のある')
add('tone.humorous', 'Humorous', 'ユーモラス')
add('tone.romantic', 'Romantic', 'ロマンチック')
add('tone.suggestive', 'Suggestive', '意味深')
add('tone.teasing', 'Teasing', 'からかい')
add('tone.witty', 'Witty', 'ウィット')
add('tts.japaneseUnavailable', 'Japanese voice not installed', '日本語の音声が未インストール')
add('tts.japaneseUnavailableDetail', 'No Japanese voice is installed on this device. Add a Japanese voice in your device text-to-speech settings to hear lines spoken.', 'この端末に日本語の音声がインストールされていません。端末の音声読み上げ設定で日本語の音声を追加すると、行を音声で聞けます。')
add('tts.speakJapanese', 'Play Japanese', '日本語を再生')
add('usageType.comeback', 'Comeback', '切り返し')
add('usageType.game', 'Game', 'ゲーム')
add('usageType.question', 'Question', '質問')
add('usageType.statement', 'Statement', '一言')
add('validation.boldnessRequired', 'Choose a boldness level.', '大胆さを選んでください。')
add('validation.englishRequired', 'English meaning is required.', '英語の意味は必須です。')
add('validation.koreanRequired', 'Korean text is required.', '韓国語は必須です。')
add('validation.usageTypeRequired', 'Choose a line type.', 'タイプを選んでください。')
