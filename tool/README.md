...
External imports (Flutter, sqflite, `dart:` libraries) are listed but not
judged. Resolving them properly would need their symbol tables, and guessing
produces false positives, which is worse than silence in a checker.

The one exception is `RE_EXPORTS`: a hand-maintained list of packages that
re-export another package wholesale, where importing both unprefixed is an
`unnecessary_import` and therefore a hard error here. It exists because
`sqflite_common_ffi` re-exports `sqflite_common/sqlite_api.dart`, importing
both looked entirely reasonable, and it cost a full Windows CI run to find out
otherwise. Add a pair to that table whenever a new package is introduced that
wraps another.

`check_lint_risks.py` approximates `prefer_final_locals`,
`always_declare_return_types`, `unawaited_futures`, `avoid_slow_async_io` and
`prefer_null_aware_operators`.

`prefer_const_constructors` is deliberately **not** approximated. Deciding
whether an expression is const-able needs real constant evaluation, and a
regex guess produces far more noise than signal — so those are left to
`flutter analyze`, which is the only thing that can answer them correctly. It is a heuristic and says
