...
# dart:io methods whose async form the analyzer rejects in favour of the
# synchronous one. Flagged by `avoid_slow_async_io`, which this project has
# enabled, so each of these is a build failure rather than a suggestion.
SLOW_ASYNC_IO = ("exists", "stat", "lastModified", "lastAccessed")

SLOW_ASYNC_RE = re.compile(
    r"\bawait\s+[\w$.()\[\]'\"]*\.(" + "|".join(SLOW_ASYNC_IO) + r")\s*\("
)


def slow_async_io_risks(code: str) -> list[str]:
    """Flags `await someFile.exists()` and friends."""
    findings = []
    for match in SLOW_ASYNC_RE.finditer(code):
        line = code[: match.start()].count("\n") + 1
        findings.append(
            f"line {line}: 'await ...{match.group(1)}()' - the analyzer wants "
            f"the sync form here (avoid_slow_async_io)"
        )
    return findings


# `x == null ? null : x.foo` is precisely what `x?.foo` means. Flagged by
# prefer_null_aware_operators, which is enabled here, so it is a build failure.
# The backreference is what makes this precise: both sides must name the same
# variable, so an unrelated ternary is never caught.
NULL_AWARE_RE = re.compile(r"(\b[\w$.]+) == null \? null : \1[.\[]")


def null_aware_risks(code: str) -> list[str]:
    """Flags an explicit null ternary that `?.` already expresses."""
    findings = []
    for match in NULL_AWARE_RE.finditer(code):
        line = code[: match.start()].count("\n") + 1
        findings.append(
            f"line {line}: '{match.group(1)} == null ? null : ...' - use "
            f"'{match.group(1)}?.' (prefer_null_aware_operators)"
        )
    return findings
...
def main() -> int:
    ...
    for path in sources:
        ...
        findings = (
            local_var_risks(code)
            + missing_return_types(code)
            + slow_async_io_risks(code)
            + null_aware_risks(code)
        )
